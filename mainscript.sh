...
gooe(){
	...
	local EXONTOT="${NEXONS}"
	local EXONLFT="${EXONTOT}"											#Exons left.
	local c															#will eventually be counting no of exons per gene.
	...
	while [ $EXONLFT -gt $LE ]; do 
		...
		echo $a $c && echo $a $c >> geneinfo								#this file contains the genes I actually processed. Usefull later for multigene alignment.
		local k=1
		for i in $(seq ${SP_counter}) ; do									#this counter neeeds to go to SP_counter number of species. depending on input.
			...
			sed -n -e "${k}p" ./prouts/output${cntr}.fa | awk -F"_" '{print ">" $2}' >> ${gooe_dir}/${a}_${i}.fasta 		#this line makes for the name to be only in the first line
			local b=0
																														#next loop concatenates all exons of the gene.
			while [[ $b -lt $c ]]; do
					#echo $b $c																							various echo messages reamained from testing the script
					temp_input_index=$(($cntr + $b))
					#echo $temp_input_index	 
					sed -n -e "${j}p" ./prouts/output${temp_input_index}.fa >> "${temp_dir}/gene_${a}_${i}" 								# makes for all the sequences of the gene to be addeed to the ne file. These files I could delete before the end of the script to reduce garbage.
					b=$((b+1))
				done
		...
		z=$((z+1))
		cntr=$(($cntr + $c))
															#everytime except from c, add one so next iterations the exon of next gene instead of the final exon of previous gene. which would have happened without adding one.
		EXONLFT=$((EXONLFT - c - 1)) 
	done
	rm ./gnxndt
...
directory_manager(){
							#This function deals with directories and files created in previous runs for successful re-runs
	echo "Preparing the working directory..."
	if [[ -f ./geneinfo ]]; then
		rm ./geneinfo
	fi

	if [[ -f ./exit_log.txt ]]; then
		rm ./exit_log.txt
	fi
